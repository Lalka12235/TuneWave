...
class UserService:
    ...
    async def authenticate_user_with_google(self, google_data: GoogleOAuthData) -> tuple[UserResponse, Token]:
        ...
        return self.user_mapper.to_response(user), Token(access_token=access_token,token_type='bearer')
    ...
    async def load_avatar(self,user: User,avatar_file: UploadFile) -> UserResponse:
        ...
        if len(content) > settings.avatar.MAX_AVATAR_SIZE_BYTES:
            logger.warning(f"Размер загруженного файла аватара ({len(content)} байт) превышает лимит ({settings.avatar.MAX_AVATAR_SIZE_BYTES} байт).")
            raise HTTPException(
                status_code=400,
