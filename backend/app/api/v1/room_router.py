...
@room.post("/room")
async def create_room(username: Annotated[str, Query()], room_settings: RoomSchema,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.create_room(username, room_settings)

@room.put("/room/{room_name}")
async def update_room(username: Annotated[str, Query()], room_name: str, room_settings: UpdRoomSchema,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.update_room(username, room_settings)

@room.delete("/room/{username}")
async def delete_room(username: str,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
...
async def join_room(username: Annotated[str, Query()], room_name: str, password: Annotated[str | None, Query()] = None,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.join_room(username, room_name, password)

@room.post("/room/{room_name}/leave")
async def leave_room(username: Annotated[str, Query()], room_name: str,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.leave_room(username, room_name)

@room.post("/room/{room_name}/kick")
async def kick_member(owner_name: Annotated[str, Query()], username: Annotated[str, Query()], room_name: str,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
...
async def ban_user_in_room(
    ...
    current_user: str = Depends(get_current_user), _ = Depends(check_authorization)
):
...
async def get_current_track(room_name: str,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.get_current_track(room_name)

@room.post("/room/{room_name}/track")
async def set_current_track(room_name: str, track: GetTrackSchema,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.set_current_track(room_name, track)


# 📃 Queue 
@room.post("/room/{room_name}/queue")
async def add_track_to_queue(username: Annotated[str, Query()], room_name: str, track: GetTrackSchema,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.add_track_to_queue(username, room_name, track)

@room.delete("/room/{room_name}/queue")
async def del_track_from_queue(username: Annotated[str, Query()], room_name: str, track: GetTrackSchema,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
    return RoomServices.del_track_from_queue(username, room_name, track)

@room.post("/room/{room_name}/queue/skip")
async def skip_track_queue(room_name: str,current_user: str = Depends(get_current_user), _ = Depends(check_authorization)):
